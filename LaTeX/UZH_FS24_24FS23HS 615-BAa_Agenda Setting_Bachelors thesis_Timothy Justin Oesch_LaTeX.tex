...
\begin{abstract}
    This paper analyses the effect of financial resources of a party during an election and the party's salient issues on the media agenda. By giving an overview of the existing pertinent literature, two premises are formulated to build a framework that connects campaign funds to the public agenda through a theorized agenda-setting process. Hypotheses in regards to party activity and party relevance in the media and the connection of party issue salience and media issue salience are derived. Using a novel dataset including party earnings for the 2023 Swiss national election for the larger chamber of parliament is used, as well as over 40'000 articles from Swiss newspapers in the 6 months leading up to the election. In order to measure the relevant party positions, recent publications by the 5 largest parties in parliament are converted to plain text. The media articles and party positions are coded into 21 issue categories through the use of an artificially intelligent process, returning an index for media issue salience (\textit{MISI}) and party issue salience (\textit{PISI}). There is supportive evidence for the connection of party budgets and their relevance in the media. The hypotheses linking party budgets and party activity and the effect of the interaction between financial resources and party issue salience on the media agenda could not be corroborated conclusively.
\end{abstract}
...
The last portrayed step from the public agenda to the electoral result is an implicit one: As shown above, the scientific debate around the influence of issue salience on vote choice is far from settled. Even though this paper does not aim to add to this controversy directly, I find it relevant to keep an eye on the bigger picture and suggest that future research traces the machinery I proposed all the way to electoral outcomes, as this is where the most practical implications in regards to policy-making lie.


\subsubsection{Putting flesh on the bone: Hypotheses.}
I am now ready to propose three hypotheses that can be derived from my framework. The mechanism from \autoref{fig:fig1} allows for three points of measurement: In a first step, I set out to test whether different levels in campaign funds are influencing partys' abilities in the agenda-setting process. My first hypothesis therefore reads: 

\begin{quote}
\sc{The Agency Hypothesis (H1)}: \textit{The higher a party’s election campaign budget, the more active they are during an election campaign.}
\end{quote}

The second point of measurement is what I would like to call the \textit{direct consequence} of the agenda setting process: If a party is a successful agent in the different political arenas of an election campaign, then they should be more visible in the public domain. In other words: If a party campaigns harder, then more people should be talking about them. Because the levels of campaign activity are correlated to campaign funds in my model, I want to stipulate the second hypothesis as follows:

\begin{quote}
    \sc{The Frequency Hypothesis (H2):} \textit{The higher a party’s election campaign budget, the more frequently they will appear in media reporting.}
\end{quote}
The third and final point of measurement is the \textit{indirect consequence} of the agenda setting process and surrounds the centrality of issues in the media agenda. If parties are competing for the issues they put at the forefront of their program to take up space in the media and campaign activity has an effect on the news’ content, then there should be a correlation between campaign budgets and media issue salience. This leads me to propose my third and final hypothesis:

\begin{quote}
    \sc{The Issue Salience Hypothesis (H3):} \textit{The higher a party’s election campaign budget, the more likely it is that the issues that are relevant for them are relevant in the media agenda as well.}
\end{quote}
...
Next, I want to describe MISI scores in our dataset descriptively: \autoref{tab:tab3} displays the relevant summary statistics for MISI scores by CAP major topic. As becomes obvious from taking a look at the standard deviations, the variance of MISI score per topic is small: With a mean standard deviation of 0.006 MISI-points and an average mean-to-standard deviation ratio over 11.5:1, the spread of our index is tight. In other words: Within the data I chose, the media did not change its focus severely. A look at \autoref{fig:fig5} confirms this as well: Only a small number of trends can be identified, notably the continuous decline of \textit{«Domestic Commerce»}, the peak of \textit{«Environment»} three months ahead of the election, the rise of \textit{«Government Operations»} towards the end of our dataset and the rapid uptick in reporting on \textit{«International Affairs»}. I will address this limitation in the concluding remarks of this thesis.
...
\subsection{Analysis of H1 and H2}
...
\autoref{tab:tab5} includes the OLS coefficients for the 7 models I ran to test the issue salience hypothesis H3. I started with an empty model (1), pitting PISI against MISI-scores within the election month, iteratively adding earnings, party controls, the interaction term and finally MISI-scores three months (5) and six months (6) before the elction, building up to the full model (7).
...
\begin{sidewaystable}
    ...
    \hline 
    \hline \\[-1.8ex] 
    \textit{Note:}  & \multicolumn{7}{r}{$^{+}$p$<$0.3; $^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
    \end{tabular} 
    }
...
\begin{enumerate}
    \item \textit{What if we just measured MISI and PISI wrongly?} It is important to start this conclusion by stating that the regression analysis I ran was insufficient to support the issue salience hypothesis. Nonetheless, the fourth and fifth point I made lead me to argue that the missing connection between party budgets, PISI and MISI might be because the data does not accurately measure the underlying concepts or it is flawed through factors out of my control. Therefore, I want to use this discovery not to argue in favor of H3, but to stipulate that there is reason to keep investigating.
