...
lm(MISI_1m ~ PISI + log(party_earnings), data = df_final) -> m2

## Add ideological and party strength controls to m3
lm(MISI_1m ~ PISI + log(party_earnings) + no_seats + lrecon + galtan, data = df_final) -> m3

## Add interaction between PISI and party_earnings
lm(MISI_1m ~ PISI * log(party_earnings) + no_seats + lrecon + galtan, data = df_final) -> m4

## Add MISI_3m as a control
lm(MISI_1m ~ PISI * log(party_earnings) + no_seats + lrecon + galtan + MISI_3m, data = df_final) -> m5

## Replace MISI_3m with MISI_6m
lm(MISI_1m ~ PISI * log(party_earnings) + no_seats + lrecon + galtan + MISI_6m, data = df_final) -> m6

## Full model
lm(MISI_1m ~ PISI * log(party_earnings) + no_seats + lrecon + galtan + MISI_3m + MISI_6m, data = df_final) -> m7
...
stargazer(
    ...
    covariate.labels = c(
        "PISI",
        "Log of Party Earnings",
        "Number of Seats",
        "Left-Right Scale",
        "GAL-TAN Scale",
        "MISI 3 months before ED",
        "MISI 6 bonths before ED",
        "PISI * Log of Party Earnings"
    ),
    ## Group models
    column.labels = c(
        "PISI",
    ...
    star.cutoffs = c(0.7, 0.1, 0.05, 0.01, 0),
    star.char = c("・", "*", "**", "***"),
